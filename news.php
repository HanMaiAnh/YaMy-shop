...
// --- Build WHERE (positional placeholders) ---
$whereParts = [];
$params = [];
...
if ($date_from !== '') {
    // assume date_from is YYYY-MM-DD; if user sends dd/mm/yyyy you'll need to convert
...
// --- COUNT total (prepare + bind positional) ---
try {
    $countSql = "SELECT COUNT(*) FROM news $whereSQL";
    $stmtCount = $pdo->prepare($countSql);
    if (!empty($params)) {
        for ($i = 0; $i < count($params); $i++) {
            // bindValue 1-based index; use STR for both dates and LIKE
...
// --- SELECT rows (bind positional + limit offset) ---
...
// build query string keep for pagination links
$keep = [];
...
<head>
    ...
    <style>
        * {margin:0; padding:0; box-sizing:border-box; font-family: 'Montserrat', sans-serif;}
        ...
        /* filter bar (single unified style) */
    ...
    font-size:15px;
    background:#fff;
    flex:0 0 auto;          /* KHÔNG giãn hết chiều ngang */
    width:350px;            /* chiều rộng bạn muốn (tuỳ chỉnh) */
    max-width:100%;         /* tránh bị tràn khi màn nhỏ */
}
