...
// kiểm tra column tồn tại (safe) — dùng để biết có thể tìm theo code/name hay không
...
/* Add button (kept generic) */
.add-btn {background:#E91E63;color:#fff;padding:10px 14px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-flex;align-items:center;height:42px;}
.add-btn:hover {background:#ff4081;}
/* Search bar */
.search-bar { margin-top:20px; display:flex; align-items:center; gap:12px; flex-wrap:wrap; }
...
/* Push the add button to the right when placed inside the form */
.add-btn-right { margin-left: auto; }
...
@media (max-width:900px){
    ...
    .add-btn-right { margin-left: 0; width:100%; text-align:center; margin-top:8px; }
}
</style>
</head>
<body>
...
<div class="content">
    ...
    <!-- Search + filter bar. Add button placed inside and pushed right via .add-btn-right -->
    <form method="get" class="search-bar" action="vouchers.php" novalidate>
        ...
        <!-- Nút thêm: được đặt trong form để cùng hàng, và được đẩy sang phải bằng margin-left:auto -->
        <a href="vouchers_add.php" class="add-btn add-btn-right">+ Thêm voucher</a>
    </form>
...
document.getElementById('btnReset').addEventListener('click', function(){
    ...
    // Reset selects to 'all' (if exists)
    form.querySelectorAll('select').forEach(s=>{
        if (s.name === 'filter') s.value = 'all';
