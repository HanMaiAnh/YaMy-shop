...
function sanitize_price_input($str) {
    $str = trim((string)$str);
    if ($str === '') return 0.0;
    // loại bỏ dấu chấm phân nghìn, giữ dấu phẩy làm thập phân
...
/**
 * Đồng bộ products.status dựa trên tổng quantity của product_variants
 * Nếu tổng > 0 => status = 1 (hiển thị)
 * Nếu tổng = 0 => status = 0 (ẩn)
 */
function autoUpdateProductStatus(PDO $pdo, int $productId): void {
    try {
        $stmt = $pdo->prepare("SELECT COALESCE(SUM(quantity),0) FROM product_variants WHERE product_id = :pid");
        $stmt->execute([':pid' => $productId]);
        $total = (int)$stmt->fetchColumn();

        $newStatus = $total > 0 ? 1 : 0;
        $update = $pdo->prepare("UPDATE products SET status = :s WHERE id = :pid");
        $update->execute([':s' => $newStatus, ':pid' => $productId]);
    } catch (PDOException $e) {
        error_log("autoUpdateProductStatus error for product {$productId}: " . $e->getMessage());
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_product'])) {
    ...
    // status: 1 = hiển thị (còn hàng), 0 = ẩn (hết hàng)
    $status           = isset($_POST['status']) ? (int)$_POST['status'] : ($product['status'] ?? 1);
    ...
    if (!$error) {
        ...
        // reload product data for form after update
        $stmt = $pdo->prepare("SELECT * FROM products WHERE id = :id");
        $stmt->execute([':id' => $productId]);
        $product = $stmt->fetch(PDO::FETCH_ASSOC);

        // Nếu admin vừa set status thủ công, hãy giữ như admin đã chọn.
        // Tuy nhiên, nếu bạn muốn auto-sync status từ variants ngay sau update product,
        // uncomment dòng dưới:
        // autoUpdateProductStatus($pdo, $productId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_variant']) && !isset($_POST['add_variant'])) {
    ...
    // Đồng bộ status dựa trên variants sau khi xóa
    autoUpdateProductStatus($pdo, $productId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_variants'])) {
    ...
    // Sau khi cập nhật/insert variants: đồng bộ status tự động
    autoUpdateProductStatus($pdo, $productId);
