...
try {
    ...
    if (!$colExists) {
        $pdo->exec("ALTER TABLE comments ADD COLUMN is_hidden TINYINT(1) NOT NULL DEFAULT 0");
    }
...
$message = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hide_comments'])) {
    $hideId = (int)$_POST['hide_comments'];
    if ($hideId > 0) {
        $stmt = $pdo->prepare("UPDATE comments SET is_hidden = 1 WHERE id = ?");
        $stmt->execute([$hideId]);
        $message = "Đã ẩn bình luận #{$hideId}.";
    }
    header("Location: comments.php?msg=" . urlencode($message));
    exit;
}
...
$sql = "
    SELECT r.id, r.product_id, r.user_id, r.rating, r.comment, r.date_comment,
    ...
    <div class="content">
        ...
        <div class="filter-bar">
            <form method="get" style="display:flex;gap:8px;align-items:center;">
                ...
                <a class="reset" href="comments.php" style="display:inline-block;padding:10px 12px;border-radius:8px;background:#ef4444;color:#fff;font-weight:700;text-decoration:none;">Reset</a>
            </form>
        </div>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?= h($r['date_comment']) ?></td>
                                <td>
                                    <div style="display:flex;gap:6px;">
                                        <!-- link tới trang chi tiết comments (dùng id sản phẩm như trước) -->
                                        <a class="btn-small btn-view" href="comments_detail.php?id=<?= (int)$r['product_id'] ?>" title="Chi tiết"><i class="fa fa-eye"></i><span>Chi tiết</span></a>

                                        <!-- form ẩn có class hide-form để JS bắt -->
                                        <form method="post" class="hide-form" style="display:inline;">
                                            <input type="hidden" name="hide_comments" value="<?= (int)$r['id'] ?>">
...
document.addEventListener('DOMContentLoaded', function(){
    document.querySelectorAll('form.hide-form').forEach(function(form){
        form.addEventListener('submit', function(e){
            e.preventDefault();
            const idInput = form.querySelector('input[name="hide_comments"]');
