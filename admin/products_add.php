...
// Config: nếu true thì sau khi insert, nếu tổng qty = 0 -> ép status = 0 (ẩn)
// Nếu bạn muốn hoàn toàn tôn trọng lựa chọn admin, set = false
define('AUTO_FORCE_HIDE_WHEN_ZERO', true);

// Helper: an toàn hóa giá (nhận số có dấu chấm/ phẩy)
function sanitize_money($v) {
    $v = trim((string)$v);
    if ($v === '') return 0.0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // lấy status do admin chọn (1 = hiển thị / còn hàng, 0 = ẩn / hết hàng)
    $posted_status    = isset($_POST['status']) ? (int)$_POST['status'] : 1;

    $variants = $_POST['variants'] ?? [];

    // kiểm tra tối thiểu 1 biến thể hợp lệ & build cleaned variants
    ...
    foreach ($variants as $v) {
        ...
        if ($size_id > 0 && $color_id > 0) {
            ...
            $price = sanitize_money($v['price'] ?? 0);
            $price_reduced = sanitize_money($v['price_reduced'] ?? 0);
            ...
            $variantsClean[] = [
                ...
                'price' => max(0.0, $price),
                'price_reduced' => max(0.0, $price_reduced),
            ];
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
            if ($_FILES['image']['error'] === UPLOAD_ERR_OK && is_uploaded_file($_FILES['image']['tmp_name'])) {
                ...
                if (strpos($mime, 'image/') !== 0) {
                    $error = "File tải lên không phải ảnh hợp lệ.";
                } else {
                    ...
                    $imagePath = uniqid('prod_', true) . '_' . $safeName . '.' . $ext;
                ...
                $error = "Lỗi upload ảnh (mã: " . intval($_FILES['image']['error']) . ")";
            }
        } else {
    ...
    if (!$error) {
        // Chính sửa: ưu tiên dùng status do admin chọn.
        // Nếu bạn bật AUTO_FORCE_HIDE_WHEN_ZERO và tổng qty = 0 -> ép ẩn
        $status_final = $posted_status;
        if (AUTO_FORCE_HIDE_WHEN_ZERO && $total_qty <= 0) {
            $status_final = 0;
        }
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO products
                  (name, description, category_id, discount_percent, is_featured, status, created_at)
                VALUES
                  (:name, :description, :category_id, :discount_percent, :is_featured, :status, NOW())
            ");
            $stmt->execute([
                ':name'             => $name,
            ...
            // Nếu bạn muốn: sau khi thêm variants vẫn đảm bảo products.status phù hợp với tổng qty
            if (!AUTO_FORCE_HIDE_WHEN_ZERO) {
                // không ép, giữ nguyên status_final
            } else {
                // đảm bảo DB trùng với status_final (trường hợp bạn muốn)
                $stmtUpdate = $pdo->prepare("UPDATE products SET status = :status WHERE id = :id");
                $stmtUpdate->execute([':status' => $status_final, ':id' => $productId]);
            }
