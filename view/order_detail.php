...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['comment_product_id'])) {
    ...
    // Kiểm tra user đã có review cho sản phẩm này chưa (bảng comments)
    ...
    if ($review_id) {
        // Cập nhật review (sử dụng cột `comment`)
        $stmtUpdate = $pdo->prepare("
            UPDATE comments
            SET comment = ?, rating = ?, date_comments = NOW()
            WHERE id = ?
        ");
        $stmtUpdate->execute([$comment, $rating, $review_id]);
        $_SESSION['success'] = 'Cập nhật đánh giá thành công.';
    } else {
        // Thêm review mới (sử dụng cột `comment`)
        $stmtInsert = $pdo->prepare("
            INSERT INTO comments (product_id, user_id, rating, comment, date_comment)
            VALUES (?, ?, ?, ?, NOW())
        ");
        $stmtInsert->execute([
            $product_id,
...
/* ============================
   LẤY REVIEW CỦA USER (NẾU CÓ) TỪ BẢNG `comments`
============================ */
$user_comments = [];
if (!empty($items)) {
