...
// Giả sử bạn dùng bảng product_images với cột image_url
// Nếu 1 sản phẩm có nhiều ảnh, bạn có thể dùng GROUP BY và MIN(pi.id) để lấy ảnh đầu tiên.
$sql_items = "
    SELECT 
        od.*,
        p.name,
    ...
    <?php if (!empty($order_items)): ?>
        <?php foreach ($order_items as $item): ?>
            <div class="order-item">
                ...
                <?php else: ?>
                    <!-- Nếu không có ảnh, có thể hiển thị ảnh mặc định hoặc bỏ img -->
