...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
    ...
    try {
        // insert đánh giá vào DB
        $ins = $pdo->prepare("
            INSERT INTO reviews (product_id, user_id, rating, content, created_at)
            VALUES (?, ?, ?, ?, NOW())
        ");
        $ins->execute([$id, $_SESSION['user_id'], $rating, $content]);
        $_SESSION['success'] = "Cảm ơn bạn đã đánh giá!";
    } catch (Exception $e) {
        $_SESSION['error'] = "Lỗi lưu đánh giá. Vui lòng thử lại.";
    }
    redirect("product-detail.php?id={$id}");
}
...
foreach ($variants as $v) {
    ...
    // chọn default size ưu tiên có hàng
    if ($defaultSize === null) {
...
$defaultVariant = $sizeMap[$defaultSize] ?? ['base_price'=>0,'final_price'=>0,'discount_pct'=>0];
...
// =============================
// Sản phẩm khác màu (cùng base name trước " - ")
// =============================
$fullName = $product['name'] ?? '';
$baseName = $fullName;
if (strpos($fullName, ' - ') !== false) {
    $baseName = explode(' - ', $fullName)[0];
}

$stmtColors = $pdo->prepare("
    SELECT 
        p2.id,
        MIN(col.name) AS color_name
    FROM products p2
    JOIN product_variants v2 ON v2.product_id = p2.id
    LEFT JOIN colors col ON v2.color_id = col.id
    WHERE SUBSTRING_INDEX(p2.name, ' - ', 1) = ?
    GROUP BY p2.id
    ORDER BY p2.id ASC
");
$stmtColors->execute([$baseName]);
$colorProducts = $stmtColors->fetchAll(PDO::FETCH_ASSOC);
foreach ($colorProducts as $cp) {
    if ((int)$cp['id'] === (int)$id && !empty($cp['color_name'])) {
        $currentColorName = $cp['color_name'];
        break;
    }
}
...
// =============================
// Đánh giá
// Trung bình + 10 đánh giá mới nhất
// =============================
...
<div class="container product-page my-5">
    ...
    <div class="product-rating d-flex align-items-center">
        ...
        <span>(<?= $avg_rating ?>/5 - <?= count($reviews) ?> đánh giá)</span>
    </div>
</div>
                ...
                <div class="product-option mb-2">
                    ...
                    <div class="d-flex flex-wrap gap-2 colors-wrap">
                        <?php foreach ($colorProducts as $cp):
                            $colorName = $cp['color_name'] ?: 'Không rõ';
                            $pid = (int)$cp['id'];
                            $isCurrent = ($pid === (int)$id);
                            $cls = 'btn btn-outline-dark btn-sm' . ($isCurrent ? ' active' : '');
                        ?>
                            <a href="product-detail.php?id=<?= $pid ?>" class="<?= $cls ?>">
                                <?= htmlspecialchars($colorName) ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                    <small class="text-muted d-block mt-1">Màu hiện tại: <strong><?= htmlspecialchars($currentColorName) ?></strong></small>
                </div>

                <!-- quantity + đã chọn trên 1 dòng -->
                    ...
                    <div class="selected-info">
                        <small class="text-muted">Đã chọn:
                            <span id="selected-info" class="text-danger fw-semibold">
                                <?= htmlspecialchars($defaultSize) ?> / <?= htmlspecialchars($currentColorName) ?> - 1 cái
                            </span>
                        </small>
                    </div>
                </div>
                ...
                <!-- hidden inputs -->
                <div id="cartData" style="display:none" data-product-id="<?= (int)$id ?>"></div>
...
<!-- Tabs: mô tả, đánh giá, related -->
<div class="container mb-5">
    <ul class="nav nav-tabs" id="productTabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="tab-desc" data-bs-toggle="tab" data-bs-target="#pane-desc" type="button" role="tab">Mô tả chi tiết</button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="tab-review" data-bs-toggle="tab" data-bs-target="#pane-review" type="button" role="tab">Đánh giá (<?= count($reviews) ?>)</button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="tab-related" data-bs-toggle="tab" data-bs-target="#pane-related" type="button" role="tab">Sản phẩm liên quan</button>
        </li>
    </ul>
            ...
            <div class="row g-4">
                <div class="col-lg-5">
                    <div class="border rounded p-3 h-100">
                        ...
                        <p class="text-muted mb-0"><?= count($reviews) ?> đánh giá cho sản phẩm này.</p>
                    </div>
                </div>

                <div class="col-lg-7">
                    <?php if (isset($_SESSION['user_id'])): ?>
                        <form method="POST" class="mb-4" aria-label="Form đánh giá sản phẩm">
                            ...
                            <button type="submit" name="submit_review" class="btn btn-dark"><i class="fa-solid fa-paper-plane"></i> Gửi đánh giá</button>
                        </form>
                    <?php else: ?>
                        <p class="text-muted">Vui lòng <a href="login.php">đăng nhập</a> để viết đánh giá sản phẩm.</p>
                    <?php endif; ?>
                        ...
                        <?php else: foreach ($reviews as $r): ?>
                            <div class="border-bottom py-3">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <small class="text-muted"><?= date('d/m/Y H:i', strtotime($r['created_at'] ?? 'now')) ?></small>
                                </div>
                        ...
                        <div class="col-6 col-md-4 col-lg-3">
                            <a href="product-detail.php?id=<?= (int)$rp['id'] ?>" class="text-decoration-none text-dark">
                                <div class="card h-100 border-0 shadow-sm related-card">
                                    <img src="<?= htmlspecialchars($imgUrl) ?>" class="card-img-top related-img" alt="<?= htmlspecialchars($rp['name']) ?>">
                                    <div class="card-body p-2">
                                        <h6 class="card-title mb-1 text-truncate"><?= htmlspecialchars($rp['name']) ?></h6>
                                        <?php if ($priceText): ?>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updatePrice() {
        ...
        if (discPt > 0 && final < base) {
            if (priceOldEl) { priceOldEl.classList.remove('d-none'); priceOldEl.textContent = formatPrice(base); }
            if (discountBadge) { discountBadge.textContent = '-' + discPt + '%'; discountBadge.classList.remove('d-none'); }
        } else {
            if (priceOldEl) { priceOldEl.textContent = ''; priceOldEl.classList.add('d-none'); }
    ...
    document.querySelectorAll('.btn-size').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            document.querySelectorAll('.btn-size').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-pressed','false'); });
            this.classList.add('active'); this.setAttribute('aria-pressed','true');
            sizeInputHidden.value = this.getAttribute('data-size');
            updateSelectedInfo(); updatePrice();
        });
    });

    // color select
    document.querySelectorAll('.btn-color, .colors-wrap a').forEach(el => {
        el.addEventListener('click', function (e) {
            // link color items are anchors -> allow navigation
            if (this.tagName.toLowerCase() === 'a') return;
            document.querySelectorAll('.btn-color').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-pressed','false'); });
            this.classList.add('active'); this.setAttribute('aria-pressed','true');
            colorInputHidden.value = this.getAttribute('data-color');
            updateSelectedInfo();
        });
    });
